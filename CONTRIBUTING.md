...
Thank you for contributing to Google Search Console MCP. This guide outlines quality expectations and the contribution process.
...
- Keep changes **small and atomic**: one commit = one logical change (prefer one file per commit when the file is independently meaningful)
- Every published commit must build and pass tests
...
- Do not regress quality: run `go test ./...` before pushing
...
```text
feat(gsc): add Search Console domain types
test(gsc): cover analytics date validation
